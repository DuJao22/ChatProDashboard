        ...
        if old_session_id:
            ...
            for item in anonymous_cart_items:
                existing_item = query_db('''
                    SELECT * FROM cart_items 
                    WHERE customer_id = ? AND product_id = ?
                ''', [customer_id, item['product_id']], one=True)
...
def process_without_ai(content_lower, session_id=None, conv_data=None):
    ...
    if any(word in content_lower for word in ['quero', 'preciso', 'manda', 'me vê', 'me da', 'me dá', 'queria', 'gostaria', 'pode', 'pedir']):
        # Extrair quantidade
        quantity_match = re.search(r'(\d+)\s*(galão|galões|galao|galoes|garrafa|garrafas|lata|latas|litro|litros|pack|packs|unidade|unidades|caixa|caixas)?', content_lower)
...
def format_orders_for_chat(orders):
    ...
    for order in orders:
        ...
        msg += f"\n• Pedido #{order['id']}\n  {status}\n  R$ {order['total']:.2f} ({date_str})"

    msg += "\n\n💡 Digite 'meus pedidos' para ver detalhes"
    return msg
...
def handle_message(data):
    ...
    # Verificar se é uma saudação e se o cliente já tem histórico
    content_lower = content.lower()
    if any(word in content_lower for word in ['oi', 'olá', 'ola', 'hey', 'hi', 'bom dia', 'boa tarde', 'boa noite']) and len(content.split()) <= 3:
        # Buscar histórico de mensagens
        message_count = query_db('SELECT COUNT(*) as count FROM messages WHERE conversation_id = ?', [conv_id], one=True)

        # Também buscar customer_id da conversa no banco de dados
        conversation = query_db('SELECT customer_id FROM conversations WHERE id = ?', [conv_id], one=True)

        if message_count and message_count['count'] > 2:  # Se já existe histórico
            # Tentar encontrar customer_id de várias fontes
            customer_id = session.get('customer_id') or conv_data.get('data', {}).get('customer_id')

            # Se ainda não tem, buscar da conversa no banco
            if not customer_id and conversation and conversation['customer_id']:
            ...
            if customer_id:
                ...
                if customer:
                    # Atualizar estado da conversa para registrado
...
def process_chat_message(session_id, content, conv_data):
    ...
    state = conv_data.get('state', 'awaiting_phone_first')
    user_data = conv_data.get('data', {})
    customer_id = user_data.get('customer_id')
    conv_id = conv_data.get('conversation_id')

    # ============ NOVOS ESTADOS DO FLUXO PHONE-FIRST ============
    # Estes devem ser processados PRIMEIRO antes de qualquer outro estado

    # Estado inicial: aguardando telefone primeiro
    if state == 'awaiting_phone_first':
        phone = ''.join(filter(str.isdigit, content))

        if len(phone) < 10 or len(phone) > 11:
            return "Hmm, esse telefone não parece certo.\n\nDigita com o DDD, tipo: 31 99999-9999"
        ...
        if existing_customer:
            ...
            # Montar endereço para confirmar
            address_lines = []
            if existing_customer['address'] and existing_customer['number']:
                street_line = f"{existing_customer['address']}, {existing_customer['number']}"
                if existing_customer['complement']:
                    street_line += f" - {existing_customer['complement']}"
                address_lines.append(street_line)

            if existing_customer['neighborhood']:
                neighborhood_line = existing_customer['neighborhood']
                if existing_customer['city'] and existing_customer['state']:
                    neighborhood_line += f" - {existing_customer['city']}/{existing_customer['state']}"
                address_lines.append(neighborhood_line)

            if existing_customer['cep']:
                address_lines.append(f"CEP: {existing_customer['cep']}")

            address_info = "\n".join(address_lines) if address_lines else "Nenhum endereço cadastrado"

            active_conversations[session_id]['state'] = 'confirming_registration'
            first_name = existing_customer['name'].split()[0] if existing_customer['name'] else existing_customer['name']
            return f"Encontrei você! 😊\n\n*Nome:* {existing_customer['name']}\n*Endereço:*\n{address_info}\n\nÉ você? Responde SIM ou NÃO"
        else:
    ...
    if state == 'confirming_registration':
        if any(word in content_lower for word in ['sim', 's', 'yes', 'sou', 'eu', 'isso', 'certo']):
            ...
            session.pop('customer_id', None)
            return "Sem problemas! Me passa o telefone correto com DDD:"
        else:
            return "Não entendi... É você? Responde SIM ou NÃO"

    # Coletando nome para novo cadastro
    if state == 'awaiting_name_new':
        ...
        active_conversations[session_id]['state'] = 'awaiting_cep_new'
        first_name = full_name.split()[0]
        return f"Prazer, {first_name}! 😊\n\nAgora me passa seu CEP:"

    # Coletando CEP para novo cadastro
    if state == 'awaiting_cep_new':
        ...
        try:
            ...
            if 'erro' not in cep_data:
                ...
                return f"✅ Achei!\n\n📍 {cep_data.get('logradouro', '')}\n{cep_data.get('bairro', '')} - {cep_data.get('localidade', '')}/{cep_data.get('uf', '')}\n\nQual o número da sua casa/apartamento?"
            else:
                return "CEP não encontrado. Confere e digita de novo?"
        except Exception as e:
    ...
    if state == 'need_phone_for_order':
        ...
        if customer:
            ...
            # Cliente não encontrado - fazer cadastro
            ...
            return "Não encontrei esse telefone cadastrado ainda.\n\nVamos fazer seu cadastro rapidinho! Qual é o seu nome completo?"
    ...
    if state == 'awaiting_phone':
        # Limpar completamente o telefone removendo todos os caracteres não numéricos
        phone = ''.join(filter(str.isdigit, content))

        # Validação básica do telefone
        if len(phone) < 10 or len(phone) > 11:
            return "😅 Esse telefone não parece certo!\n\nDigite seu número com o código da cidade (DDD).\n\nExemplo: 31 99999-9999"
        ...
        if existing_customer:
            ...
            # Monta o endereço completo de forma mais legível
            address_lines = []
            if existing_customer['address'] and existing_customer['number']:
                street_line = f"{existing_customer['address']}, {existing_customer['number']}"
                if existing_customer['complement']:
            ...
            active_conversations[session_id]['state'] = 'confirming_address'
            first_name = existing_customer['name'].split()[0] if existing_customer['name'] else existing_customer['name']
            return f"Oi, {first_name}! Que bom te ver! 😊\n\nSeu endereço de entrega:\n{address_info}\n\nTá certo? Responde SIM ou NÃO"
        else:
            # Cliente novo - inicia cadastro
            active_conversations[session_id]['data']['phone'] = phone
            active_conversations[session_id]['state'] = 'awaiting_cep'
            return "Ótimo! Agora preciso do seu endereço.\n\nQual é o seu CEP?"

    # Confirmando endereço de cliente existente
    if state == 'confirming_address':
        if any(word in content_lower for word in ['sim', 's', 'yes', 'confirmo', 'ok', 'certo', 'isso']):
            # Cliente confirmou endereço, perguntar como quer continuar
            active_conversations[session_id]['state'] = 'choosing_order_method'
            existing_customer = query_db('SELECT * FROM customers WHERE id = ?', 
                                        [user_data.get('customer_id')], one=True)

            if existing_customer:
                first_name = existing_customer['name'].split()[0] if existing_customer['name'] else 'amigo'
                return f"Perfeito, {first_name}! Endereço confirmado!\n\nComo você prefere fazer seu pedido?\n\n1️⃣ Continuar aqui no chat\n2️⃣ Ir para a loja online\n\nDigite 1 ou 2"
            else:
                active_conversations[session_id]['state'] = 'awaiting_name'
            ...
            # Cliente não respondeu claramente
            return "Não entendi... O endereço tá certo? Responde SIM ou NÃO"

    # Escolhendo método de pedido (chat ou loja)
    if state == 'choosing_order_method':
...
📍 Agradecemos a sua preferência! Em breve, um entregador da Ariguá Distribuidora estará no seu endereço: R. Rio Xingu, 592.
...
Precisa de algo mais? Estou aqui para ajudar! 😊"""
    ...
    if state == 'browsing_products':
        if any(word in content_lower for word in ['finalizar', 'fechar', 'pedido', 'carrinho', 'checkout', 'pagar']):
            cart_items = []
            if customer_id:
                cart_items = query_db('''
            ...
            if not cart_items:
                return "[SHOW_PRODUCTS]Seu carrinho está vazio!\n\nAdicione produtos clicando no botão *+* nos cards acima."

            total = sum(item['price'] * item['quantity'] for item in cart_items)
            items_text = "\n".join([f"• {item['quantity']}x {item['name']} - R$ {(item['price'] * item['quantity']):.2f}" for item in cart_items])

            active_conversations[session_id]['state'] = 'awaiting_order_confirmation'
            active_conversations[session_id]['data']['pending_order_items'] = [
                {'product_id': item['product_id'], 'name': item['name'], 'quantity': item['quantity'], 'price': item['price']}
                for item in cart_items
            ]
            active_conversations[session_id]['data']['pending_order_total'] = total

            return f"🛒 *Seu Carrinho:*\n\n{items_text}\n\n*Total: R$ {total:.2f}*\n\nConfirma o pedido? Digite SIM ou NÃO"

        if any(word in content_lower for word in ['ajuda', 'dúvida', 'help', 'como', 'comprar']):
            return "[SHOW_PRODUCTS]Para comprar:\n\n1️⃣ Clique no botão *+* para adicionar produtos\n2️⃣ Use as categorias para filtrar\n3️⃣ Clique em *Finalizar Pedido* quando terminar\n\nOu digite *ver carrinho* pra ver o que já escolheu!"

        if any(word in content_lower for word in ['ver carrinho', 'meu carrinho', 'o que tem', 'carrinho']):
            cart_items = []
            if customer_id:
                cart_items = query_db('''
        ...
        if any(word in content_lower for word in ['voltar', 'sair']):
            active_conversations[session_id]['state'] = 'registered'
            return "Voltando... Posso te ajudar em algo mais?"

        # Se digitou qualquer outra coisa enquanto navega produtos, mostrar os cards novamente
        return "[SHOW_PRODUCTS]Use os cards acima para escolher produtos!\n\nClique no botão *+* para adicionar ao carrinho.\n\nQuando terminar, digite *finalizar*"
    ...
    return "Oi! Me passa seu telefone com DDD pra eu te identificar:"
