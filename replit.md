...
- Cadastro de clientes via chat com fluxo otimizado:
  1. Começa pedindo telefone para identificação
  2. Se cliente existe: confirma dados e oferece escolha (chat ou loja)
  3. Se cliente novo: cadastro rápido (nome → CEP → número → complemento)
  4. Após autenticação: exibe cards de produtos com filtros de categoria
- Cards de produtos interativos com botão + para adicionar ao carrinho
- Botão "Finalizar Pedido" redireciona para confirmação
...
- `GET /api/chat/products` - Lista produtos e categorias para o chat
